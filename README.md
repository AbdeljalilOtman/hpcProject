...
```
HPC-RESOURCES-MANAGER/
├── app.py                    # Main entry point for the Dash app
├── assets/                 # UI-related files
...
└── scripts/                  # Shell scripts 
    ├── script.sh             # Script to run on simlab
```
...
1. **Frontend**: 
   ...
   - The `styles.css` file in the `assets` folder is used for styling the app.
2. **Backend**: 
...
4. **Scripts**:
   - The `scripts/script.sh` file  containes the mechanism to retrieve available resources from a given partition.
